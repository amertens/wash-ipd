...
hm_agg <- heatmap_plot(hm_df_agg %>% filter(!(target %in% unique(hm_df_agg$target)[1:5])) %>% droplevels(), colours=colours) 
...
hm1 <- heatmap_plot(hm_df_path, colours=colours) 
...
hm2 <- heatmap_plot(hm_df_path %>% filter(!(target %in% unique(hm_df_path$target)[1:9])) %>% droplevels(), colours=colours) 
...
hm1_abund <- heatmap_plot(hm_df_path_abund, colours=colours) 
...
hm_mst_abund <- heatmap_plot(hm_df_mst_abund, colours=colours) 
