...
__Supplementary table 1.__
Baseline covariates by study. Note Odigari 2016 is not included as all samples are from village water sources and did not have associated household covariates, so all Odigari 2016 estimates are unadjusted`r cmt(".", "Arrange and harmonize covariate levels for the table. This doesn't effect the analysis because it is done study-by-study, but affects the table formatting", "Andrew", "Date")` 


```{r, echo=F, results = 'asis'}
FitFlextableToPage_landscape(flextable(tab2))
```

__Supplementary table 2.__
Unadjusted and adjusted results by study, sample, and target.

```{r, echo=F, results = 'asis'}
FitFlextableToPage_landscape(flextable(clean_res_tab(unadj_RR,adj_RR)))
```
