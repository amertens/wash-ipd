...
clean_res  <- function(res, outcome="binomial"){
  if(outcome=="binomial"){
    res$est.ci = paste0("PR=",round(as.numeric(res$RR),1)," (95% CI: ",round(res$ci.lb,1),", ",round(res$ci.ub,1),")")
    res <- res %>% ungroup() %>%
    ...
    res$est.ci = paste0("Difference=",round(as.numeric(res$coef),1)," (95% CI: ",round(res$ci.lb,1),", ",round(res$ci.ub,1),")")
    res <- res %>% ungroup() %>%
...
Different columns denote separation by "AND" in the search string. 
...
## Table S4. Prevalence of pathogens by sample type tested in each study 
...
## Table S5. Prevalence of microbial source tracking markers by sample type tested in each study
...
FitFlextableToPage_landscape(flextable(target_presence_long_MST))
...
Unadjusted and adjusted results by study, sample, and target.
...
FitFlextableToPage_landscape(flextable(tabs7))
```
...
Baseline covariates by study. Note that Odigari et al. 2016 is not included as data shared from this study were from village water sources and did not have associated covariates from individual households; therefore all estimates for this study are unadjusted`r cmt(".", "Arrange and harmonize covariate levels for the table. This doesn't effect the analysis because it is done study-by-study, but affects the table formatting. Will sort covariate levels better", "Andrew", "Date")` 
