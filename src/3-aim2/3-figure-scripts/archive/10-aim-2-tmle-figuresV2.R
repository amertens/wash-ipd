...
adj_RR_no_prescreen <- readRDS(file=here("results/adjusted_aim2_pooled_no_prescreen.Rds")) %>% mutate(analysis="No Screen") 

adj_RR <- bind_rows(adj_RR, adj_RR_tmle, adj_RR_no_prescreen)
...
adj_RR <- adj_RR %>%
  mutate(analysis = factor(analysis, levels=c("No Screen","TMLE","GLM"))) %>% 
...
table(adj_RR$analysis)
table(is.na(adj_RR$study))
...
adj_RR %>% filter(study=="Pooled", target %in% c("Any pathogen"), Y=="haz", sample=="any sample type")
adj_RR %>% filter(study=="Pooled", target %in% c("Any pathogen"), Y=="diar7d", sample=="any sample type")
