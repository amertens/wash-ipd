...
res_diar_adj <- d %>% group_by(study, sample, target) %>%
  do(res=aim2_tmle(., Ws = Wvars, forcedW=c("age", "hhwealth"), outcome="diar7d", exposure="pos", study=.$study[1], sample=.$sample[1], target=.$target[1], family="binomial")) 
...
res_haz_adj <- d %>% group_by(study, sample, target) %>%
  do(res=aim2_tmle(., Ws = Wvars_anthro, forcedW=c("age", "hhwealth"), outcome="haz", exposure="pos", study=.$study[1], sample=.$sample[1], target=.$target[1], family="gaussian")) 
