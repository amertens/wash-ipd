...
res_diar_adj <- d %>% group_by(study, sample, target) %>%
  do(aim2_tmle(., Ws = Wvars, forcedW=c("age", "hhwealth"), outcome="diar7d", exposure="pos", study=.$study[1], sample=.$sample[1], target=.$target[1], family="binomial")) 
...
saveRDS(res_diar_adj, file=here("results/tmle_aim2_res_diar.Rds"))


res_haz_adj <- d %>% group_by(study, sample, target) %>%
  do(aim2_tmle(., Ws = Wvars_anthro, forcedW=c("age", "hhwealth"), outcome="haz", exposure="pos", study=.$study[1], sample=.$sample[1], target=.$target[1], family="gaussian")) 
...
saveRDS(res_haz_adj, file=here("results/tmle_aim2_res_haz.Rds"))
...
saveRDS(fullres_adj, file=here("results/tmle_aim2_res.Rds"))
