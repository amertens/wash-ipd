...
mapsan <- readRDS(paste0(dropboxDir, "Data/WBK/clean-mapsan-diar.RDS")) %>% select( -c(haz, whz, waz, round, Kkasc, KKtrc, starts_with("gpp_"))) %>% filter(!is.na(diar7d)) %>% select(dataid,clusterid,hhid, child_date, diar7d, sex, age) %>% distinct()
...
env_fuhr <- merge_ch(env_fuhr, fuhr) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_boehm <- merge_ch(env_boehm, boehm) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_kwong <- merge_ch(env_kwong, kwong) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_stein <- merge_ch(env_stein, wbk) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_cap <- merge_ch(env_cap, cap) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_cap2 <- merge_ch(env_cap2, cap2) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_hol <- merge_ch(env_hol, hol) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
env_gv <- merge_ch(env_gv, gv) %>% distinct(study, trial, sampleid, dataid, hhid, clusterid, sample, env_date, child_date, target, pos, .keep_all = T)
