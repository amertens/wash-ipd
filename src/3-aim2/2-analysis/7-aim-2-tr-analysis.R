
rm(list=ls())
source(here::here("0-config.R"))

dfull <- readRDS(paste0(dropboxDir,"Data/merged_env_CH_data_clean.rds")) %>% filter(!is.na(pos))

#make one observation per child health outcome (instead of multiple for many targets/samples)
colnames(dfull)
dfull$target <- factor(dfull$target)
dfull$target <- relevel(dfull$target, ref="Any pathogen")
dfull$sample <- factor(dfull$sample)
dfull$sample <- relevel(dfull$sample, ref="any sample type")
d <- dfull %>% group_by(study, dataid, hhid, childid, tr, is.na(haz), is.na(diar7d)) %>% 
  arrange(sample, target, rev(age)) %>% distinct(study, dataid, hhid, childid, tr, is.na(haz), is.na(diar7d), .keep_all=T)

#note! check sample sizes--- WBB too small
table(d$trial)
dhaz <- d %>% filter(!is.na(haz))
table(dhaz$trial)

Wvars = c("sex","age","hfiacat","momage","hhwealth", "Nhh","nrooms","walls", "roof", "floor","elec","dadagri","landacre","landown", "momedu")         
Wvars_anthro = c("sex","age_anthro","hfiacat","momage","hhwealth", "Nhh","nrooms","walls", "roof", "floor","elec","dadagri","landacre","landown", "momedu")         

table(d$tr)
d$tr <- ifelse(d$tr=="Intervention",1,0)


fullres_unadj <- NULL
res_diar_unadj <- d %>% group_by(trial) %>%
  do(aim2_glm(., Ws = NULL,  outcome="diar7d", exposure="tr", study=.$study[1], sample=.$sample[1], target=.$target[1], family="binomial", minN_thres=1)) 
res_diar_unadj$sparse <- ifelse(is.na(res_diar_unadj$RR), "yes", "no")
res_diar_unadj$RR[is.na(res_diar_unadj$RR)] <- 1


res_haz_unadj <- d %>% group_by(trial) %>% filter(study!="Odagiri 2016") %>% droplevels() %>%
  do(aim2_glm(., Ws = NULL, outcome="haz", exposure="tr", study=.$study[1], sample=.$sample[1], target=.$target[1], family="gaussian")) 
res_haz_unadj$sparse <- ifelse(is.na(res_haz_unadj$coef), "yes", "no")
res_haz_unadj$coef[is.na(res_haz_unadj$coef)] <- 0

fullres_unadj <- bind_rows(res_diar_unadj, res_haz_unadj)


fullres_adj <- NULL
res_diar_adj <- d %>% group_by(trial) %>%
  do(aim2_glm(., Ws = Wvars, forcedW=c("age", "hhwealth"), outcome="diar7d", exposure="tr", study=.$study[1], sample=.$sample[1], target=.$target[1], family="binomial", minN_thres=1)) 
res_diar_adj$sparse <- ifelse(is.na(res_diar_adj$RR), "yes", "no")
res_diar_adj$RR[is.na(res_diar_adj$RR)] <- 1


res_haz_adj <- d %>% group_by(trial) %>% filter(study!="Odagiri 2016") %>% droplevels() %>%
  do(aim2_glm(., Ws = Wvars_anthro, forcedW=c("age_anthro", "hhwealth"), outcome="haz", exposure="tr", study=.$study[1], sample=.$sample[1], target=.$target[1], family="gaussian")) 
res_haz_adj$sparse <- ifelse(is.na(res_haz_adj$coef), "yes", "no")
res_haz_adj$coef[is.na(res_haz_adj$coef)] <- 0

fullres_adj <- bind_rows(res_diar_adj, res_haz_adj)


saveRDS(fullres_unadj, file=here("results/unadjusted_aim2_tr_res.Rds"))
saveRDS(fullres_adj, file=here("results/adjusted_aim2_tr_res.Rds"))


#-NOTE! Check WBK main trial results
#Note that the WBK data was from the S and WSH arms
# We collected 2107 soil samples from February 2015 to July 2016; 898 from the active control
# arm, 613 from the sanitation arm, and 596 from the combined WSH arm of the WASH Benefits
# trial (Fig 1).


