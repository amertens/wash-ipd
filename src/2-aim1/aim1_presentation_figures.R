...
p_pres_1 <-  ggplot(data = mydf, (aes(x=study, y=RR, group=sample_cat, color=sample_cat, shape=sparse))) + 
    ...
    scale_y_continuous(#breaks=scales::breaks_pretty(c(0.25, 0.5,1, 2, 4, 8)),
      breaks=c(0.0625, 0.125,.25, .5,0.75,1, 1.5, 2, 4, 8, 16), 
      trans='log10', 
      labels = c("1/16","1/8","1/4", "0.5","0.75","1", "1.5", "2", "4", "8", "16")
    ) + coord_flip(ylim=c(0.42,2.5))+
...
pres_adj_1 <- p_adj_1 %+% theme(legend.position = "none")
ggsave(pres_adj_1, file = paste0(here::here(),"/figures/pngs/pres_aim1_full_adj.png"), width = 8, height = 5)


p_adj_2 <- adj_RR %>% 
  filter(target %in% c("Any MST", "Any human MST","Any animal MST")) %>%
  base_plot(lab_width=20) %+% theme(legend.position = "none")
ggsave(p_adj_2, file = paste0(here::here(),"/figures/pngs/pres_aim1_full_adj_mst.png"), width = 8, height = 5)
